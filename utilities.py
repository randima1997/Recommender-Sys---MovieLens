...
def user_CF_recommendation(user_similarity, user_means, user_item_centered, userId):
    ...
    centered_values = user_item_centered.loc[top_sim_user_indices]                          # Extracts movie ratings df using top_sim_user_indices
    


    # The calculation below predicts the rating and creates an ndarray of shape (1, num_movies) 
    pred_rating = mean + ((similarity_series.to_numpy().reshape(1,20))@(centered_values.to_numpy().reshape(20,-1)))/(similarity_series.sum())

    # Returns the resulting rating predictions for user as a dataframe
    return pd.DataFrame(pred_rating.reshape(-1,1), index= user_item_centered.columns, columns= [userId])